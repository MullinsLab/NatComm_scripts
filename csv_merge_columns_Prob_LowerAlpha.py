...
for file in csv_files:
    ...
    df = pd.read_csv(file, header=None, usecols=range(7))
    
    # Extract the desired column (adjust the index as needed)
    extracted_columns.append(df.iloc[:, 5])  # 1 for second column
...
merged_df.to_csv('merged_columns_ProbLowerAlpha.csv', index=False, header=False)
