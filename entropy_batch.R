...
if (length(args)==0) {
  stop("Input alignment folder must be supplied. \n", call.=FALSE)
} else {
  ...
  dir.create("tsv_files")
  
  for (f in files) {

    datadf <-  seqinr::read.fasta(paste0(args,"/",f), seqtype= "AA", as.string = T)
    ...
    write.table(df, paste0("tsv_files/", gsub("\\.[^\\.]+$", "", f), "_positionInfo.tsv"), quote=FALSE, row.names=FALSE, sep="\t")
    cat(paste0("Data frame exported as '", gsub("\\.[^\\.]+$", "", f), "_positionInfo.tsv' \n"))
  }
}
